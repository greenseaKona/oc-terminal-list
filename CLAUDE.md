...
## 퀵바 기본값 — 멀티플렉서 키는 안 싣는다 (2026-09-03)

한때 tmux 프리픽스 키 여섯 개(`T·c` `T·%` `T·"` `T·z` `T·o` `T·n`)를 기본으로 심었다.
근거는 "이 앱의 팬은 대개 tmux 안" 이었는데, 실제로는 **폰의 짧은 바에서 정작 자주 쓰는
것을 스크롤 뒤로 밀어냈다.** 거기 있어야 하는 것은 엔터·지우기·화살표·^C 다. 새 윈도우를
여는 일은 앱의 탭 UI 가 이미 한다. 필요하면 설정 프리셋에서 골라 넣는다(`TMUX_KEYS` 는
그 프리셋용으로 남아 있다).

기본 바는 이것뿐이다: **빠른입력 · 화살표 · ESC TAB ⏎ ⌫ · ^C · CTRL · 복사/붙여넣기.**

- ⚠️ **엔터(`⏎`)와 지우기(`⌫`)가 오래 빠져 있었다.** 소프트 키보드로 칠 때는 안 아쉬운데,
  퀵바만으로 다루면 **줄을 끝낼 수도 고칠 수도 없다.** 둘 다 기본에 넣었다.
- ⚠️ **조합키는 `^C` 와 `CTRL` 둘만.** 나머지(^A ^E ^U ^W ^R ^L ^D ^Z, ALT, PgUp/PgDn)는
  프리셋으로 내렸다. 그 둘을 남기는 이유는 **각각이 유일한 길**이라서다: `^C` 는 소프트
  키보드를 안 열고 중단하는 유일한 길(바에 알파벳 키가 없어 CTRL 토글로는 못 만든다),
  `CTRL` 은 나머지 조합을 만드는 유일한 길이다.
- **여기에 키를 더하기 전에 "이게 없으면 폰에서 무엇을 못 하나" 를 먼저 답할 것.**
  스크롤 뒤로 밀린 키는 없는 키다.
- 길게 누르면 연타된다(`utils/keyRepeat`, 420ms 뒤 80ms). ⚠️ 반복은 `kind: 'send'` 에만
  붙는다 — kind 를 바꾸면 조용히 안 된다.
- ⚠️ **기본값만 바꾸면 이미 저장된 사용자에게는 영영 반영되지 않는다**(`mobileKeys` 는 첫
  실행에 저장된다). `migrateMobileKeys` 가 저장된 바를 **한 번** 훑어 옛 `mux_` 키를 걷고
...
- **원격 tmux 도 따라간다.** 원격 우편함을 걷으러 이미 호스트당 5초에 한 번 나가는
  왕복(`remote_outbox`) 에 활성 tmux pane cwd 를 같이 싣는다. 추가 SSH 연결·요청·폴링은
  없고, 백엔드 캐시가 경로 변경분만 기존 SSE 로 보낸다. 원격 경로는 이미 절대경로라
  프론트에서 바로 쓰고 확인용 `/cwd` 요청을 다시 보내지 않는다. tmux 가 아닌 `none`
  pane 은 원격 tmux 경로를 알 수 없으므로 기존 표시를 유지한다.
...
⚠️ **표식만으로는 안 된다 — 브리지가 없으면 아무도 안 읽는다.** 스캐너는 그 팬의 **WS
브리지 안에** 산다. 브라우저가 그 탭에 붙어 있지 않으면 브리지가 아예 없고, 찍힌 표식은
그대로 사라진다(로그도 안 남는다). 배경에서 도는 에이전트가 정확히 그 상태다 — 실제로
"보냈는데 안 갔다, 확인할 방법도 없다" 의 원인이 이것이었다(2026-09-03 실측: 보내는 팬이
`attached=0` 이었다).

→ **우편함(`@itl_outbox`)이 그 조건을 없앤다.** `itl` 이 표식을 찍으면서 같은 JSON 을 tmux
사용자 옵션에도 세우고, 에이전트 상태 폴링이 **같은 tmux 호출의 칸 하나**로 걷어 간다
(왕복 0). 걷은 쪽은 **먼저 비우고** 배달한다 — 배달이 느리면 다음 틱이 같은 통을 또 집는다.
- 🔐 우편함은 표식보다 **더** 믿을 만하다: 화면에 찍는 것은 그 팬을 지나가는 어떤 출력이든
  할 수 있지만, tmux 옵션을 세우려면 그 소켓에 닿아야 한다(같은 OS 사용자). 그래서 여기엔
  열쇠를 요구하지 않는다.
- ⚠️ **둘 다 나가므로 중복을 접어야 한다.** 붙어 있는 팬은 표식과 우편함으로 같은 것을 두 번
  보낸다 → 라우터가 `(보낸팬, n)` 으로 접는다. 난수가 없으면(옛 클라이언트) 접지 않는다.
- **원격 팬이 보내는 것도 된다**(`remote_outbox.py`, 2026-09-03). 그쪽 tmux 는 남의 기계라
  위 폴링이 못 보므로, 탭 상태에 원격 팬이 있는 호스트만 **호스트당 왕복 하나**로 훑어
  비우고 걷는다(`ssh_pool` 로 연결 재사용, 보는 사람 있으면 5s·없으면 30s).
  - ⚠️ **비우는 것과 걷는 것이 한 명령 안에 있어야 한다.** 나누면 다음 주기가 같은 통을 또 집는다.
  - ⚠️ **Tailscale 호스트도 걷는다.** `ssh_pool` 을 직접 열면 `open_connection` 이 tailscale
    인증을 몰라 던져 그 호스트는 한 번도 안 걷혔다(조용히). `run_remote_cmd_pooled` 가 갈래를
    안다. 그리고 같은 왕복에 그 호스트 세션들의 `@pane_addr` 를 다시 새긴다 — 붙어 있지 않은
    팬·Tailscale 팬은 브리지 stamp 가 닿지 않아 번호가 밀리면 낡은 주소가 남고, 그 호스트에서
    `itl send 1.3` 은 로컬 탐색이 그 옵션으로 먼저 푸니 엉뚱한 팬에 Enter 까지 눌린다.
  - ⚠️ **붙어 있어도 비운다.** 한때 "붙어 있으면 브리지가 처리하니 건너뛴다" 로 짰는데,
    `itl` 은 **언제나 두 통로로** 내보내므로 그 통이 영영 안 비워진다 → 나중에 옛말로
    되살아난다. 겹치는 것은 라우터의 nonce 가 접는다.
  - ⚠️ **tmux `-F` 는 `\t` 를 안 풀어 준다.** 큰따옴표 안의 `\t` 는 두 글자로 나가고 `read` 가
    못 쪼개 **아무것도 안 걷힌다 — 조용히.** 실제로 이걸로 한 번 헛돌았다. 진짜 탭을 박는다.
- ⚠️ **설치된 `itl` 은 낡는다 — 그리고 화면이 그걸 말해야 한다.** 배달 경로는 매번 현재
  파일을 stdin 으로 밀지만, 호스트에 **설치된** 사본은 그때 그 판본이다. 우편함이 없는 옛
  사본은 표식만 찍고 조용히 끝나는데 화면은 "설치됨" 초록불이었다 — 갱신할 이유를 볼 수가
  없다. 그래서 확인 명령이 **지문(sha256)** 을 함께 찍고, 백엔드가 지금 밀 파일과 비교해
  `outdated` 를 붙인다(`host_tools.is_outdated`). 화면은 "옛 버전" 칩 + [업데이트] 버튼.
  - ⚠️ 확인 명령은 여전히 **그 도구를 실행하지 않는다** — `command -v` 로 자리를 찾고
    `sha256sum` 으로 읽기만 한다(이 파일 머리말의 규칙 2).
  - ⚠️ **지문을 못 읽으면 `None`(모름)이지 "최신" 이 아니다.** `sha256sum` 이 없는 기계가
    있다. 모르는 것을 최신으로 그리면 그 호스트는 영영 안 고쳐진다.
- 이름이 세 곳에 있다: `cli/itl` · `itl_channel.OUTBOX_OPTION` · `PANE_FORMAT`. 어긋나면
  조용히 아무 일도 안 일어난다 — `test_우편함_옵션_이름이_세_곳에서_같다` 가 잠근다.
...
- **성공도 알린다**(`[itl] 8.2 로 전달됨`). ⚠️ 이게 없으면 **침묵이 두 가지**를 뜻한다 —
  잘 갔거나, 표식이 아예 안 주워졌거나. 표식 통로는 한 방향(팬 → 백엔드)이라 보낸 쪽은
  그 둘을 스스로 구별할 수 없다. 실제로 "전달됐는지 확인할 방법이 없다" 가 첫 신고였다.
  - **진단은 로그가 한다**: 성공은 `itl deliver …` (INFO), 실패는 `itl 배달 실패` (WARNING),
    열쇠 불일치는 `itl sentinel rejected` (WARNING). **셋 다 없으면 표식이 애초에 안 나갔다**
    (그 팬에 `itl` 이 없거나, 열쇠가 없어 CLI 가 거절했거나).
- **통지는 입력줄이 아니라 팬의 화면에 쓴다**(`itl notify` → `pane_tty`). ⚠️ 타이핑해 두면
...
- 보낸 이는 페이로드가 아니라 **브리지가 세션에서 되짚는다**(사칭 방지). 원격은
  **호스트 + 세션명**으로 — 기본 세션명 `mobile` 은 호스트마다 같다. 상한: 8192B, 10초 5회
  (표식 통로는 스캐너가, 우편함 통로는 라우터가 보낸 팬 단위로 같은 값을 건다).
  재생 억제(`_seen`)는 **파일로 남는다**(`data/.itl-seen`). tmux 세션도 화면의 표식 줄도
  백엔드보다 오래 사는데, 재시작 뒤 첫 re-attach 의 재그리기가 그 줄을 새 스캐너에 다시
  넣는다 — 메모리만 믿으면 그때 한 번 더 배달된다.
- ⚠️ **테스트는 자기가 도는 팬의 tmux 에 닿지 않는다.** pytest 를 이 앱의 팬 안에서 돌리면
  `TMUX`/`TMUX_PANE` 이 그 세션을 가리키고, `cli/itl` 의 `put_outbox` 는 그 값으로 **진짜**
  우편함을 세운다. 실제로 `send_app_addr("1.2","hi")` 를 부르는 테스트가 스위트를 돌릴 때마다
  다른 호스트의 팬 1.2 에 "hi" 를 배달했다(2026-09-06). `tests/conftest.py` 가 그 환경변수를
  지우고, 재생 파일·열쇠 파일도 tmp 로 보낸다.
