...
분할은 서브탭으로 접히고, 키 툴바가 `Esc`·`Tab`·`Ctrl+C`·방향키·붙여넣기를 제공합니다. iPhone/iPad에서는 한글 조합을 커서 위치에 표시하고 확정된 글자만 전송해 터미널에 직접 입력할 수 있습니다. 긴 문장은 빠른 입력에서 작성할 수도 있습니다.
...
원하는 것을 설치합니다. 내장은 **tmux** 와 **itl** 이고 나머지는 직접 추가합니다. 이 앱이
...
> 설치 여부를 확인하는 명령은 그 도구를 **실행하면 안 됩니다** — `command -v tmux` 는
...
- 긴 문장은 명령을 다 쓴 뒤 한 번에 보낼 수 있습니다. iPhone/iPad 한글 직접 입력은 터미널 안에서도 지원합니다(화면 읽기 모드에서는 기본 xterm 입력 사용).
...
- **일반 → 스크롤 → 터미널 스크롤바 표시**: 오른쪽 스크롤바를 켜거나 끕니다. 드래그로 이전 출력을 보고, 맨 아래로 내리면 실시간 출력으로 돌아갑니다. 로컬·원격 tmux 기록도 지원합니다.
- **일반 → 스크롤 → 스크롤시 입력내용보기**: 위 옵션 바로 아래에 있습니다(기본 꺼짐). 위로 스크롤하면 화면 상단의 답변 구간에 해당하는 질문을 표시하고 맨 아래에서는 숨깁니다. Codex·Claude의 질문 표시(`›`, `❯`)를 기준으로 터미널 기록에서 찾습니다. 질문창을 누르면 질문의 원래 위치로 이동하며, 펼치기·접기는 별도 버튼으로 동작합니다. 긴 질문은 펼칠 수 있으며 스크롤바를 꺼도 동작합니다. tmux에 기록이 남아 있으면 새로고침 후에도 찾을 수 있습니다. 질문 표시가 없거나 기록이 잘린 구간에서는 숨깁니다. tmux 미사용 시에는 최근 명령 기록도 참조해 화면에서 확인된 질문의 원문을 보여줍니다. 옵션을 켠 상태에서 제출한 입력은 기존 최근 명령 저장소를 함께 사용합니다(서버: 500자 이하, 30일 / 이 기기: 최대 32,768자, 최근 5개). 명령만 남아 있고 답변 화면 기록이 없으면 위치를 추정하지 않습니다. 최근 명령 보기 버튼은 다시 입력할 내용을 고르는 용도로 유지됩니다.
