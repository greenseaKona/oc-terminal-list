      ...
      <style>{`
        html, body {
          ...
          /* ⚠️ 이 배경이 보이는 순간이 있다. 사파리가 페이지 상자를 실제로 보이는 영역보다
             작게 그리면(주소창 전환 뒤 다시 안 그리는 상태) 그 아래로 캔버스 배경이 드러난다
             — 그게 하단의 빈 띠다. 상자 크기는 우리가 못 정하지만 색은 정할 수 있으므로
             테마 배경으로 칠해 앱의 밑단처럼 보이게 한다. */
          background: ${currentTheme.ui.bg};
        }

        /* index.html 의 부트 CSS 와 같은 값이어야 한다 — 두 벌이다.
           fixed 를 빼면 iOS 최초 로딩에서 앱이 상단 크롬 뒤로 딸려 올라간다. */
        /* ⚠️ index.html 부트 CSS 와 같은 값이어야 한다 — 두 벌이다.
           fixed 를 빼면 iOS 최초 로딩에서 앱이 상단 크롬 뒤로 딸려 올라간다. */
        #root {
          position: fixed;
          inset: 0;
