...
- **General → Scroll → Show input while scrolling**: directly below the scrollbar option, off by default. Pins the question belonging to the answer at the viewport top; hidden at the bottom. Finds Codex/Claude prompt markers (`›`, `❯`) in terminal history, independently of scrollbar visibility. Long questions can be expanded. Retained tmux history works after page reload; sections without a recognized or retained question have no preview. Without tmux, matching Recent commands entries supply the original input text. Submissions made with the option enabled share the existing command store (server: up to 500 characters, 30 days; this browser: last 5 entries, up to 32,768 characters each). Command timestamps never substitute for missing answer positions. The Recent commands button remains available for reinserting earlier input.
